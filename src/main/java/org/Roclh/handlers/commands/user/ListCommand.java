...
public class ListCommand extends AbstractCommand<SendMessage> implements WithCallbackStack {
    ...
    public SendMessage handle(CommandData commandData) {
        ...
        return MessageUtils.sendMessage(commandData.getMessageData())
                .text("Shadowsocks users from " + (pageNumber * defaultPageSize) + " to " + (Math.min((long) (pageNumber + 1) * defaultPageSize, allUsersSize)) + " users:\n" +
                        users.stream().map(userModel -> {
                                    StringBuilder s = new StringBuilder(userModel.toFormattedString()).append("\n");
                                    bandwidthService.getRule(userModel.getUserModel().getTelegramId())
                                            .ifPresent(b -> s.append(b.toFormattedString()).append("\n"));
                                    contractService.getContract(commandData.getMessageData().getTelegramId())
                                            .ifPresent(c -> s.append(c.toFormattedString()).append("\n"));
                                    return s.toString();
                                })
    ...
    public CallbackStack getCallbackStack() {
        return CallbackStack.of("user")
                .forCommand(getCommandNames().get(0), EmojiConstants.BOOK + " " + i18N.get("callback.user.user.inline.button.list.of.all.users"))
                .withLocalizedCallbackKey(EmojiConstants.WRENCH + " " + i18N.get("callback.user.user.inline.button.manage.users"))
